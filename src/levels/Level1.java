...
public class Level1 {
    ...
    public static void main(String[] args) {
        ...
        employees.stream().distinct().filter(t -> t.getSubject() == Subject.JAVA)
                .sorted(Comparator.comparingInt(Teacher::getSalary).reversed())
                .forEach(System.out::println);
        ...
        Teacher highestPaidTeacher = employees.stream().filter(t -> t.getName().startsWith("m"))
                .peek(t -> t.setSalary(t.getSalary() + 200))
                .max(Comparator.comparingInt(Teacher::getSalary))
                .orElse(null);

        System.out.println("Highest Paid Teacher after the increase: " + highestPaidTeacher);
    }
}
